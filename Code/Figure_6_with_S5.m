...
% Matlab code to generate Figure 6 for manuscript titled "The Eco-Epidemiological Dynamics of Evolutionary Rescue in a Host Metapopulation"
...
trans_ratio=18:0.25:25; 

mig_rate=0.0000005:0.0000013:0.000037;

for i=1:length(trans_ratio) %transmission rate of RT
    for jj=1:length(mig_rate)  %ratio of transmission rate that determines the transmission of WT
...
for t=2:t1
     for j=0:N-1
        ...
        %define newborns for each genotype susceptible
...
dlmwrite('SIRmodel_WT_cycles_avepop_aveInfect_Trans_vs_Mig.csv',XXW,'-append');

XXR = [i,jj,cyclesR1,cyclesR2,cyclesR3,cyclesR4,ave_popR1,ave_popR2,ave_popR3,ave_popR4,I_R1,I_R2,I_R3,I_R4];
dlmwrite('SIRmodel_RT_cycles_avepop_aveInfect_Trans_vs_Mig.csv',XXR,'-append');
...
filename = sprintf('Trans_Ratio %d vs Mig %d.png',i,jj);
