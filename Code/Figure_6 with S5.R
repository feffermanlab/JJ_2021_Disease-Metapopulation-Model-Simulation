## =========================================================================================================================================================
##
## R code to generate Figure 6 and S5 in Appendix 2 for manuscript titled "The Eco-Epidemiological Dynamics of Evolutionary Rescue in a Host Metapopulation"
...
set_here()
##The csv. files are generated by the Matlab code named as "Figure_6_with_S5.m"
cyc_RT_matr<-read.csv("SIRmodel_RT_cycles_avepop_aveInfect_Trans_vs_Mig.csv",header=TRUE)
cyc_WT_matr<-read.csv("SIRmodel_WT_cycles_avepop_aveInfect_Trans_vs_Mig.csv",header=TRUE)
...
##define axises
x_axis=seq(18,25,0.25) #x_axis is Trans_RT

y_axis=seq(0.000001,0.000037,0.0000018) #y_axis is Mig_rate
...
tiff("Figure_6.tiff", width=10,height=10, units='in',res=600)


p1 <- ggplot(cyc_RT_patch30, aes(x_axis[X1],y_axis[X1.1])) + geom_tile(aes(fill = X0.2))+ guides(fill = guide_colorbar(title="Cycle Num"))+geom_text(aes(label = round(Patch.30,4)),size=4)+theme(legend.title = element_text(size=10))


p1+xlab("Trans_Ratio")+ylab("Mig_Rate")+ scale_fill_gradient2(low = 'steelblue', mid='white',high = 'red' ,
                                                           
                                                           midpoint=median(cyc_RT_patch30[,3]), space = "rgb", na.value = "grey50", guide = 
...
tiff("Figure_S5a.tiff", width=10,height=10, units='in',res=600)
...
tiff("Figure_S5b.tiff", width=10,height=10, units='in',res=600)
